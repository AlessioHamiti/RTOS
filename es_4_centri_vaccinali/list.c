...
ItemType FindMax(LIST l, int availability) {
    LIST tmp = l;
    ItemType t, m;
    m.patients = 0;
    int curr_max = 0;
    ...
    while (!isEmpty(tmp)){
        t = tmp->item;
        if ( t.entity == 1 && t.patients >= curr_max && t.patients <= availability ){
            curr_max = t.patients;
            m = t;
        }
        tmp = tmp->next;
...
LIST EnqueueFirst(LIST l, ItemType item) {
    ...
    t = NewList();
    t = EnqueueLast(t, item);
    LIST tmp = l;
    while (!isEmpty(l)){
        t = EnqueueLast(t, *getHead(l));
        l = DequeueFirst(l);
    }
...
void PrintItem(ItemType item) {
    if ( item.entity == 0 ){
        printf( "nome: %s, entità: %d, disponibilità: %d, minimo: %d\n", item.name, item.entity, item.availability, item.minimum );
    } else if ( item.entity == 1 ){
        printf( "nome: %s, entità: %d, pazienti al giorno: %d\n", item.name, item.entity, item.patients );
    }
}

...
ItemType* FindGuide(LIST l, int visitors) {
    LIST tmp = l;
    ItemType *t;
    ItemType *m = NULL;
    int sub = 99999;
    ...
    while (!isEmpty(tmp)){
        t = &tmp->item;
        if ( visitors <= t->max_visitors && (t->max_visitors - visitors) < sub){
            sub = t->max_visitors-visitors;
            m = t;
        }
        tmp = tmp->next;
    }
...
LIST EnqueueOrdered(LIST l, ItemType item) {
    ...
    // Lista vuota o da inserire in testa
    ...
    LIST current = l;
    while (current->next != NULL && itemCompare(item, current->next->item) >= 0) {
...
void PrintItem(ItemType item) {
    if ( item.type == 0 ){
        printf( "Guida %s (%d) -> min: %d, max: %d\n", item.name, item.type, item.min_visitors, item.max_visitors );
    } else {
        printf( "Visitatori (%d) -> %d\n", item.type, item.visitors );
    }
}
